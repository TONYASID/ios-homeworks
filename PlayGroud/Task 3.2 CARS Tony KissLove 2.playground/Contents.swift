...
// создаем расширение для протокола Car и оно говорит что AudiDealerShip не подписан на протокол Car это потому что в протоколе Car есть хранимые свойства. Типа если создать протокол с одними методами и сделать расширение класса то на это расширение не будет ругаться?
extension AudiDealerShip: SpecialOffer {
    func addEmergencyPack() {
        for (index, value) in stockCars.enumerated() {
            ...
            print("The car in stock cars \(value) add EmergencyPack") // Работает
        }
        for (index, value) in showroomCars.enumerated() {
            showroomCars[index].accessories.append("fire stopper")
            showroomCars[index].accessories.append("Aid")
            print("The car in show room \(value) add EmergencyPack") // Работает
        }
    }
    func makeSpecialOffer() {
...
//audiDeallership.showroomCars.append(audiZ5)
//audiDeallership.showroomCars.append(audiS8)
//audiDeallership.showroomCars.append(audiA6)
//audiDeallership.stockCars.append(audiS8)
//audiDeallership.stockCars.append(audiA6)
audiDeallership.stockCars
audiDeallership.showroomCars
//audiDeallership.addToShowroom(audiA6)
//audiDeallership.addToShowroom(audiZ5)
//audiDeallership.showroomCars.append(audiA6)
//audiDeallership.showroomCars.append(audiZ5)
audiDeallership.makeSpecialOffer()
